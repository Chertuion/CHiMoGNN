...
class myProteinToToken(object):
    ...
    def __init__(self,keys,
        ...
        self.return_tensors = return_tensors
        self.preprocess = lambda x: " ".join(x)
        self.tokenizer = BertTokenizer.from_pretrained(model, max_length = self.max_length)
    ...
    def __call__(self, input):
        ...
        output = self.tokenizer(input,
                                ...
                                padding=self.padding,
                                max_length=self.max_length)
        output['input_ids'] = F.pad(output['input_ids'], pad=(0, self.max_length-output['input_ids'].size(1)), mode='constant', value=0)
        output['input_ids'] = F.normalize(output['input_ids'].float(), p=2, dim=1)
        output['attention_mask'] = F.pad(output['attention_mask'], pad=(0, self.max_length-output['attention_mask'].size(1)), mode='constant', value=0)
        return output