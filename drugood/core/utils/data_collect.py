...
class Collater(object):
    ...
    def collate(self, batch, samples_per_gpu=1):
        ...
        if isinstance(batch[0], DataContainer):
            ...
            # data_batch = {
            #     key: self.collate([d[key] for d in batch], samples_per_gpu) if key not in self.exclude_keys else [d[key] for d in batch]
            #     for key in batch[0]
            # }
            ...
            for key in batch[0]:
                if key not in self.exclude_keys:
                    data_batch[key] = self.collate([d[key] for d in batch], samples_per_gpu)
                else:
                    data_batch[key] = [d[key] for d in batch]
