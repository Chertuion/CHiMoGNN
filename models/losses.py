...
def get_contrastive_loss_with_positive_and_negative_batch(original_embeddings, positive_embeddings, negative_embeddings, temperature=2.75):
    # Compute cosine similarity between original and positive embeddings
    positive_similarities = F.cosine_similarity(original_embeddings, positive_embeddings, dim=-1)

    # Compute cosine similarity between original and negative embeddingsß
    negative_similarities = F.cosine_similarity(original_embeddings, negative_embeddings, dim=-1)

    # Concatenate positive and negative similarities
    similarities = torch.cat((positive_similarities.unsqueeze(1), negative_similarities.unsqueeze(1)), dim=1)

    # Compute logits with temperature scalingç
    logits = similarities / temperature

    # Create target labels
    targets = torch.zeros(original_embeddings.size(0)).long().to(original_embeddings.device)
